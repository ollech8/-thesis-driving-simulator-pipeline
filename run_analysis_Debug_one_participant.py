# -*- coding: utf-8 -*-
"""
Run analysis for a single participant across ALL relevant conditions (Egocar only),
skip Training/Baseline, skip rows with missing files,
and write ONE combined CSV output in the code directory (or current working directory).
"""

import os
import pandas as pd
import all_analysis_functions_Debug as aaf  # make sure this is your debug functions file
...
# Only this participant AND triggered_by == Egocar
meta_rows = df_meta[
...
if meta_rows.empty:
    raise ValueError(f"לא נמצאו רשומות עבור {participant_id} עם triggered_by=Egocar")

# Output: one file for all conditions
output_path = os.path.join(os.getcwd(), f"{participant_id}_analysis_ALL_conditions.csv")
failed_path = os.path.join(os.getcwd(), f"{participant_id}_failed_conditions.csv")

all_results = []
failed_conditions = []


def record_fail(condition, map_type, step, err):
...
for _, meta_row in meta_rows.iterrows():
    ...
    if cond in ["training", "baseline"]:
        print(f"⏭️ מדלג על {participant_id} - {condition}")
        continue
    ...
    if pd.isna(kin_file) or str(kin_file).strip() == "":
        print(f"⏭️ מדלג: {participant_id} | {condition} | חסר KinematicFile")
        record_fail(condition, map_type, "meta_check", "Missing KinematicFile")
        continue

    if pd.isna(gps_file) or str(gps_file).strip() == "":
        print(f"⏭️ מדלג: {participant_id} | {condition} | חסר GPSFile")
        record_fail(condition, map_type, "meta_check", "Missing GPSFile")
        continue
    ...
    try:
        ...
        print("After add_traffic_light_junction_phase:", df_with_distances_light.shape if df_with_distances_light is not None else "None")
        ...
        print("After load_transcription_data:", df_transcripts.shape if df_transcripts is not None else "None")

        
        df_with_text = aaf.add_transcription_to_events(df_with_ttc, df_transcripts)
        print("After add_transcription_to_events:", df_with_text.shape if df_with_text is not None else "None")


        df_with_final_comments = aaf.add_comment_flag(df_with_text)
        print("After add_comment_flag:", df_with_final_comments.shape if df_with_final_comments is not None else "None")

        df_with_final_comments = aaf.add_start_comment_column(df_with_final_comments, df_transcripts, round_to=1)
        print("After add_start_comment_column:", df_with_final_comments.shape if df_with_final_comments is not None else "None")
...
if all_results:
    ...
    print("\n⚠️ לא נוצר פלט (כל התנאים נכשלו או דולגו).")

# Save failures report (optional but recommended)
if failed_conditions:
    pd.DataFrame(failed_conditions).to_csv(failed_path, index=False)
