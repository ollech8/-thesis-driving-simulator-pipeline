"""
run_ml_models_sliding.py
------------------------
Trains XGBoost and Random Forest on sliding window datasets (1s-5s, step=1s).
Identical logic to run_ml_models.py but reads sliding_windows_Xs_clean.csv files.

Key design decisions:
  - Group split by participant (Id) — no participant in both train and test
  - Stratified by has_feedback per participant
  - Same train/test split applied across all window sizes
  ...
  - Per-participant breakdown printed for first window size only
"""
...
# ─── Step 1: Participant split (based on 1s sliding dataset) ─────────────────
header("Determining participant split (based on sliding 1s dataset)")

df_ref = pd.read_csv(os.path.join(DATA_DIR, "sliding_windows_1s_clean.csv"))
...
# Stratified 80/20 split within each has_feedback stratum
...
# ── Train/test participant summary ────────────────────────────────────────────
...
# ─── Step 2: Categorical distributions ───────────────────────────────────────
header("Categorical column distributions (sliding 1s dataset)")
...
def preprocess(df, train_ids, test_ids):
    ...
    return train_df, test_df, y_train, y_test, train_cols
...
def evaluate(model, X_test, y_test, model_name, window_size):
    ...
    print(f"    Positive — mean: {pos_probs.mean():.3f}  "
          f"min: {pos_probs.min():.3f}  max: {pos_probs.max():.3f}")
    print(f"    Negative — mean: {neg_probs.mean():.3f}  "
...
for w in WINDOW_SIZES:
    header(f"Sliding window size: {w}s  (step=1s)")

    path = os.path.join(DATA_DIR, f"sliding_windows_{w}s_clean.csv")
    ...
    print(f"  Positive windows:        {n_pos:,} / {n_total:,}  "
          f"({n_pos/n_total*100:.2f}%)")
    print(f"  Imbalance ratio:         {(n_total-n_pos)/max(n_pos,1):.1f}:1")

    # Per-participant table for first window size only
    if w == WINDOW_SIZES[0]:
        print(f"\n  Per-participant breakdown (shown once for {w}s window only):")
        print(f"  {'Id':<15} {'n_events':<12} {'events_with_fb':<18} "
              f"{'n_windows':<12} {'pos_windows':<13} {'pos_rate':>10}")
        ...
        for pid, p_df in df.groupby("Id"):
            ...
            print(f"  {str(pid):<15} {p_events:<12} {p_fb_events:<18} "
                  f"{p_windows:<12} {p_pos:<13} {p_rate:>10}")
        print()

    X_train, X_test, y_train, y_test, train_cols = preprocess(
    ...
    scale_pos      = round(n_neg_train / max(n_pos_train, 1), 2)
    ...
    for feat, imp in fi_xgb.nlargest(10).items():
...
header("COMPARISON TABLE — Sliding windows, all sizes × both models")

results_df = pd.DataFrame(all_results)
results_df = results_df.sort_values(["window_sec", "model"]).reset_index(drop=True)

print("\n  ── Threshold-independent ──")
print(results_df[["model", "window_sec", "AUC_ROC", "Avg_Prec"]].to_string(index=False))

print("\n  ── At optimal threshold ──")
print(results_df[["model", "window_sec", "best_threshold",
                   "F1_optimal", "Prec_optimal", "Rec_optimal",
                   "TP", "FP", "TN", "FN"]].to_string(index=False))

print("\n  ── At default threshold (0.50) ──")
print(results_df[["model", "window_sec",
                   "F1_default", "Prec_default", "Rec_default"]].to_string(index=False))
